...
export default async function ComparePage({ searchParams }: { searchParams: Promise<{ tools?: string }> }) {
  ...
  return (
    <><header className="page-header"><div className="site-container"><span className="eyebrow">Evaluation sheet</span><h1 className="display page-title" style={{ fontSize: "clamp(3.6rem, 8vw, 6.6rem)" }}>Compare tools by workflow fit.</h1><p className="lede">Scores appear only where the evidence pass exists. Qualitative fields and machine-readiness signals remain comparable across the directory.</p></div></header><section className="wide-container section"><CompareExplorer tools={tools} initialSlugs={initial} /></section></>
  );
}
