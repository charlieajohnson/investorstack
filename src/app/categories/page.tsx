...
export default async function CategoriesPage({ searchParams }: { searchParams: Promise<{ stage?: string }> }) {
  ...
  return (
    <>
      <header className="page-header"><div className="site-container"><span className="eyebrow">Directory</span><h1 className="display page-title" style={{ fontSize: "clamp(3.6rem, 8vw, 6.8rem)" }}>Browse the operating stack.</h1><p className="lede">Seven initial categories across sourcing, execution, portfolio work and the AI operating layer. Coverage expands only after the current set is useful.</p></div></header>
