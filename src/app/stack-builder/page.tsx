...
export default async function StackBuilderPage() {
  ...
  return (
    <><header className="page-header"><div className="site-container page-header-grid"><div><span className="eyebrow">Guided operating ledger</span><h1 className="display page-title" style={{ fontSize: "clamp(3.3rem, 7vw, 6.3rem)" }}>Build your operating stack.</h1><p className="lede">Map the firm profile, workflow needs and current tools one decision layer at a time. See what is working, fill what is missing, and preserve incumbents where the evidence supports them.</p></div><div className="surface" style={{ padding: 22 }}><p className="display" style={{ fontSize: "1.4rem", margin: 0 }}>Start with the firm. Map the work. Preserve what is working. Fill only the gaps that matter.</p></div></div></header><section className="wide-container section"><StackBuilder categories={categories} tools={tools} /></section></>
  );
}
